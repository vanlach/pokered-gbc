...
.beginSorting
	...
	call .ldHLbagorbox
	ld c, 0 ; relative to wBagItems, this si where we'd like to begin swapping
.loopCurrItemInBag
...
.hasItem ; c contains where to swap to relative to the start of wBagItems
	...
	ld [hSwapTemp], a
	ld a, [hl]
	ld [hSwapItemID], a ; [hSwapItemID] is second item ID
	inc hl
	ld a, [hld]
	ld [hSwapItemQuantity], a ; [hSwapItemQuantity] is second item quantity
	ld a, [de]
	ld [hli], a ; put first item ID in second item slot
	inc de
	ld a, [de]
